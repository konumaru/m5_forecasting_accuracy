...
class WRMSSEForLightGBM(WRMSSEEvaluator):
    ...
    def set_series_weight_for_fobj(self, train_ids):
        fobj_weight, fojb_sclae = self.get_series_weight(train_ids)
        self.custom_fobj_weight = 2 * np.square(fobj_weight) / fojb_sclae
...
def get_decayed_weights(train_data, days_keep_weight=0):
    max_train_d = train_data['d'].str.replace('d_', '').astype('int').max()
    weight_map = np.flip(np.concatenate([
        [1 for _ in range(days_keep_weight)],
        [1 - 0.0005 * i for i in range(days_keep_weight, max_train_d)]
    ], axis=0)).round(4)
    weight_map = {f'd_{i+1}': weight_map[i] for i in range(max_train_d)}
    decayed_weight = train_data['d'].map(weight_map).values
...
def run_evaluation():
    ...
    valid_pred = lgb_model.predict(eval_data[features])
