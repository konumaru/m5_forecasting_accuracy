...
def run_split_data():
    ...
    lower_eval_date = '2016-03-27'  # '2016-04-24'
...
class LGBM_Model():
    ...
    def predict(self, data):
        model = self.model
        return np.expm1(model.predict(data, num_iteration=model.best_iteration))  # v05010
...
def train_model():
    ...
    categoricals = df.dtypes[df.dtypes == 'category'].index.tolist()
    # Split Train data.
    ...
    if use_weight:
        ...
        train_weight = (1.3 * train_decayed_weight)
...
def train_group_models():
    ...
    models = []
    for i, u_id in enumerate(UNIQUS):
        print(f'\n\nGroup ID: {u_id}, {i+1}/{len(UNIQUS)+1}')
        df = pd.read_pickle('features/all_train_data.pkl')
        df = df[df['id'].str.contains(u_id)]
        ...
        if u_id in ['HOBBIES_1', 'HOBBIES_2']:
        ...
        # Save Importance
        IMPORTANCE_PATH = f'result/importance/{VERSION}/{u_id}.png'
        os.makedirs(f'result/importance/{VERSION}', exist_ok=True)
        lgb_model.save_importance(filepath=IMPORTANCE_PATH, figsize=(18, 25))
        # Add Model
        group_models[group_id][u_id] = lgb_model
    return group_models
...
def run_evaluation():
    scores = {}
    lgb_models = load_pickle(MODEL_PATH)[group_id]
    ...
    for u_id in UNIQUS:
        is_groups = eval_data['id'].str.contains(u_id)
        valid_pred[is_groups] = lgb_models[u_id].predict(eval_data.loc[is_groups, features])
...
def run_submission():
    ...
    for u_id in UNIQUS:
        is_groups = submit_data['id'].str.contains(u_id)
        submit_pred[is_groups] = lgb_models[u_id].predict(submit_data.loc[is_groups, features])
