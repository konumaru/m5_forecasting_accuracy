...
@cache_result(filename='SalesTimesSellprice_lag_and_roll', use_cache=True)
def SalesTimesSellprice_lag_and_roll():
    # Define variables and dataframes.
    shift_days = 28
    use_cols = ['id', 'sales', 'sell_price']
    srd_df = pd.read_pickle('features/melted_and_merged_train.pkl')[use_cols]
    srd_df['sales_times_price'] = srd_df['sales'] * srd_df['sell_price']
    grouped_df = srd_df.groupby('id')
    dst_df = pd.DataFrame()
    # Creat Features
    agg_funcs = {}
    ...
    agg_funcs[f'{target}_rolling_skew_t30'] = grouped_df[target].transform(
        lambda x: x.shift(shift_days).rolling(30).skew())
    agg_funcs[f'{target}_rolling_kurt_t30'] = grouped_df[target].transform(
        lambda x: x.shift(shift_days).rolling(30).kurt())
...
def run_create_features():
    ...
    feat_funcs = [
        sales_lag_and_roll,
        SalesTimesSellprice_lag_and_roll,
